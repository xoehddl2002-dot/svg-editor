...
class SvgCanvas {
  ...
  constructor(
    ...
    this.deleteSVG = () => { };
    this.cloneSVG = () => { };
    this.imageLoadingComplete = (result, msg) => { return { result ,msg} };

    this.imageLoadingCount = 0;
    this.imageLoadingTimeoutCount = 0;
  ...
  getMobileBtnFunc() {
    return this.mobileBtnFunc;
  }

  setMobileBtnFunc(value) {
    this.mobileBtnFunc = value;
  }
  ...
  lastHistoryRemove() {
    this.undoMgr.lastHistoryRemove();
  }

  lastHistory() {
    this.undoMgr.lastHistory();
  }
  ...
  convertToPath2(elem) {
    ...
    const attrs = {
      fill: elem.fill,
      "fill-opacity": elem.fill_opacity,
      stroke: elem.stroke,
      "stroke-width": elem.stroke_width,
      "stroke-dasharray": elem.stroke_dasharray,
      "stroke-linejoin": elem.stroke_linejoin,
      "stroke-linecap": elem.stroke_linecap,
      "stroke-opacity": elem.stroke_opacity,
