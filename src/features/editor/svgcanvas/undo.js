...
import { getRotationAngle, getBBox as utilsGetBBox, setHref, getStrokedBBoxDefaultVisible } from './utilities.js'
import { isGecko } from '../common/browser.js'
import { transformPoint, transformListToTransform } from './math.js'

const { UndoManager, HistoryEventTypes } = hstry
    ...
    handleHistoryEvent(eventType, cmd) {
      ...
      if (eventType === EventTypes.BEFORE_UNAPPLY || eventType === EventTypes.BEFORE_APPLY) {
        ...
        if (cmdType === 'MoveElementCommand') {
          ...
          if (cmd.elem.tagName === 'title' && cmd.elem.parentNode.parentNode === svgCanvas.getSvgContent()) {
            draw.identifyLayers()
          }
    ...
    if (oldval !== String(newValue)) {
      ...
      if (
        ...
        (String(newValue).startsWith('url') ||
          (['font-size', 'font-family', 'x', 'y'].includes(attr) && elem.textContent))
      ) {
        elem = ffClone(elem)
      }
