...
export class RemoveElementCommand extends Command {
   ...
   * @param {string} [text] - An optional string visible to user related to this change
   */
  constructor(elem, oldNextSibling, oldParent, text,func) {
    super();
    this.elem = elem;
...
export class UndoManager {
  ...
  addCommandToHistory(cmd) {
    ...
    if (this.undoStack.length > this.maxSize) {
      this.undoStack = this.undoStack.splice(this.undoStack.length - this.maxSize, this.undoStack.length);
    }
    this.undoStackPointer = this.undoStack.length;
  }

  lastHistory() {
    return this.undoStack[this.undoStackPointer-1]
  }
    ...
    while (i--) {
      ...
      if (changes[attrName] !== elem.getAttribute(attrName)) {
        batchCmd.addSubCommand(
          new ChangeElementCommand(elem, changes, attrName)
        );
      }
    }
