...
import { getParents, getClosest } from './jgraduate/Util.js'

const { InsertElementCommand, RemoveElementCommand, ChangeElementCommand, BatchCommand } = hstry
...
export const init = (canvas) => {
  ...
  svgCanvas.addSvgString = addSvgString
  svgCanvas.replaceSvgString = replaceSvgString
...
const setSvgString = (xmlString, preventUndo) => {
  ...
  try {
    ...
    if (elements.length > 0) {
      ...
      svgCanvas.imageLoadingComplete(true)
    }
...
const replaceSvgString = (useEl, xmlString, func) => {
  const dataStorage = svgCanvas.getDataStorage()
  let j
  let ts
  let w = 0
  ...
  try {
    ...
    const symbolId = svgCanvas.getHref(useEl)
    const oldSymbol = useEl.ownerDocument.querySelector(symbolId)
    const defs = oldSymbol.parentElement
    ...
    //useEl.setAttribute("transform", ts);
    if (w && h) {
      useEl.setAttribute('width', w)
      useEl.setAttribute('height', h)
    }
...
const addSvgString = (elem, xmlString) => {
  const dataStorage = svgCanvas.getDataStorage()
  let j
  let ts
  let useEl
  ...
  try {
    ...
    // Look for symbol and make sure symbol exists in image
    if (svgCanvas.getImportIds(uid) && svgCanvas.getImportIds(uid).symbol) {
      const parents = getParents(svgCanvas.getImportIds(uid).symbol, '#' + elem.id)
      if (parents?.length) {
        useExisting = true
      }
    }
    ...
    if (useExisting) {
      ...
      symbol = svgCanvas.getDOMDocument().createElementNS(NS.SVG, 'symbol')
      const defs = elem.ownerDocument.createElementNS(NS.SVG, 'defs')
      if (elem.firstChild) {
        // first child is a comment, so call nextSibling
        elem.insertBefore(defs, elem.firstChild.nextSibling)
...
const embedImage = (src) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    imgI.addEventListener('load', (e) => {
      svgCanvas.imageLoadingTimeoutCount = svgCanvas.imageLoadingTimeoutCount + 1
      ...
      ctx && ctx.drawImage(e.currentTarget, 0, 0)
      ...
      try {
        ...
        cvs.width = 1
        cvs.height = 1
        const releaseCtx = cvs.getContext('2d')
        releaseCtx && releaseCtx.clearRect(0, 0, 1, 1)
      }
      svgCanvas.setGoodImage(src)
      if (svgCanvas.imageLoadingCount == svgCanvas.imageLoadingTimeoutCount) {
        svgCanvas.imageLoadingComplete(true)
      }
      resolve(svgCanvas.getEncodableImages(src))
    })
    imgI.addEventListener('error', (e) => {
      const message = `이미지를 불러올 수 없습니다.`
      svgCanvas.imageLoadingComplete(false, message)
    })
    svgCanvas.imageLoadingCount = svgCanvas.imageLoadingCount + 1
 ...
 */
const exportPDF = async (WindowName, outputType = isChrome() ? 'save' : undefined) => {
  const res = svgCanvas.getResolution()
  ...
  iframe.onload = () => {
    ...
    setTimeout(() => {
      ...
      html2canvas(iframedoc.body, { useCORS: true, allowTaint: true }).then((canvas) => {
        ...
        obj.output = doc.output(outputType, outputType === 'save' ? WindowName || 'svg.pdf' : undefined)
      ...
      if (!fillStrokeElems.length) {
        ...
        if (!tmpFillStrokeElems.length) {
          ...
          if (
            ...
            tmpFillStrokeElems[0].getAttribute('gradientUnits') === 'userSpaceOnUse'
          ) {
            fillStrokeElems = svgContent.querySelectorAll(
              '[fill="url(#' + tmpFillStrokeElems[0].id + ')"],[stroke="url(#' + tmpFillStrokeElems[0].id + ')"]',
            )
          } else {
            return
          }
