...
import { assignAttributes, getElement, getBBox as utilsGetBBox } from './utilities.js'
...
export const textActionsMethod = (function () {
  ...
  function setCursor(index) {
    ...
    const startPt = ptToScreen(charbb == undefined ? 0 : charbb.x, textbb == undefined ? 0 : textbb.y)
    const endPt = ptToScreen(charbb == undefined ? 0 : charbb.x, textbb == undefined ? 0 : textbb.y + textbb.height)
  ...
  function setSelection(start, end, skipInput) {
    ...
    const dstr =
      'M' + tl.x + ',' + tl.y + ' L' + tr.x + ',' + tr.y + ' ' + br.x + ',' + br.y + ' ' + bl.x + ',' + bl.y + 'z'
  ...
  return /** @lends module:svgcanvas.SvgCanvas#textActions */ {
    ...
    toSelectMode(selectElem) {
      ...
      if (!curtext?.textContent.length) {
        ...
        //svgCanvas.setTextContent(".");
        //svgCanvas.deleteSelectedElements();
      }
      /*else {
        if (curtext?.textContent.startsWith(".")) {
          curtext.textContent = curtext?.textContent.substr(1);
        }
      }
      */
    ...
    init(_inputElem) {
      ...
      for (i = 0; i < len; i++) {
        const c = curtext.textContent.split('')[i]
        const d = { x: 100, y: 100 }
        if (c == ' ') {
          if (i == 0) {
            const elem = svgCanvas.getSelectedElements()[0]
            if (elem.getAttribute('x')) {
              d.x = elem.getAttribute('x')
            }
            if (elem.getAttribute('y')) {
              d.y = elem.getAttribute('y')
            }
          } else {
            d.x = chardata[i - 1].x + chardata[i - 1].width
            d.y = chardata[i - 1].y + chardata[i - 1].height
          }
        }

        const start = c == ' ' ? d : curtext.getStartPositionOfChar(i)

        end = c == ' ' ? d : curtext.getEndPositionOfChar(i)
