...
const setDocumentTitleMethod = (newTitle) => {
  ...
  if (!docTitle) {
    ...
    svgCanvas
      .getSvgContent()
      .insertBefore(docTitle, svgCanvas.getSvgContent().firstChild);
...
const setResolutionMethod = (x, y) => {
  ...
  if (x !== w || y !== h) {
    ...
    batchCmd.addSubCommand(
      new ChangeElementCommand(svgCanvas.getSvgContent(), {
        width: w,
        height: h,
      })
    );
    ...
    svgCanvas
      .getSvgContent()
      .setAttribute("viewBox", [0, 0, x / zoom, y / zoom].join(" "));
    batchCmd.addSubCommand(
      new ChangeElementCommand(svgCanvas.getSvgContent(), {
...
const setBBoxZoomMethod = (val, editorW, editorH) => {
  ...
  switch (val) {
    ...
    case "layer":
      bb = getStrokedBBoxDefaultVisible(
        getVisibleElements(svgCanvas.getCurrentDrawing().getCurrentLayer())
      );
      break;
    default:
      return undefined;
...
const setGradientMethod = (type) => {
  if (
    !svgCanvas.getCurProperties(type + "_paint") ||
    svgCanvas.getCurProperties(type + "_paint").type === "solidColor"
  ) {
    return;
  }
...
const removeTextDecorationMethod = (value) => {
  ...
  if (selected?.tagName === "text" && !selectedElements[1]) {
    ...
    svgCanvas.changeSelectedAttribute(
      "text-decoration",
      actualValues.replace(value, "").trim()
    );
  }
  if (selectedElements.length > 0 && !selectedElements[0].textContent) {
    svgCanvas.textActions.setCursor();
...
const setBackgroundMethod = (color, url) => {
  ...
  if (color === "chessboard") {
    if (!bgPattern) {
      bgPattern = svgCanvas
        .getDOMDocument()
        .createElementNS(NS.SVG, "foreignObject");
