  ...
  Color: function (init) {
    ...
    function val(name, value, context) {
      ...
      switch (name.toLowerCase()) {
        case "ahex":
        case "hex": {
          const ret = ColorMethods.hexToRgba(
            (value && (value.ahex || value.hex)) || value || "none"
          );
          val.call(
            that,
            "rgba",
            {
              ...
              g: ret.g,
              b: ret.b,
              a: name === "ahex" ? ret.a : !isNullish(a) ? a : 255,
            },
            context
          );
          break;
...
export function jPickerMethod(
  ...
  const that = elem;
  const settings = sets;
  if (that.nodeName.toLowerCase() === "input") {
  ...
  if (settings.window.expandable) {
    ...
    content.innerHTML = `<span class="Icon" id="jq-ae-Icon">
      ...
      <span class="Image" id="jq-ae-Image" title="${i18next.t(
        "config.open_color_picker"
      )}">&nbsp;</span>
  ...
  const isLessThanIE7 =
    Number.parseFloat(navigator.appVersion.split("MSIE")[1]) < 7 &&
  ...
  function activeColorChanged(ui, context) {
    if (isNullish(context) || (context !== colorBar && context !== colorMap))
      positionMapAndBarArrows.call(that, ui, context);
    setTimeout(function () {
    ...
    if (context !== colorMap) {
      switch (settings.color.mode) {
        ...
        case "a": {
          ...
          colorMap.val(
            "xy",
            { x: (hv && hv.h) || 0, y: 100 - (!isNullish(hv) ? hv.v : 100) },
            context
          );
          break;
        }
        case "v": {
          ...
          colorMap.val(
            "xy",
            { x: (hs && hs.h) || 0, y: 100 - (!isNullish(hs) ? hs.s : 100) },
            context
          );
          break;
        }
  ...
  function updatePreview(ui) {
    try {
      ...
      activePreview.style.backgroundColor =
        (all && "#" + all.hex) || "transparent";
      setAlpha.call(
        that,
        activePreview,
        (all && toFixedNumeric((all.a * 100) / 255, 4)) || 0
      );
    } catch (e) {
      /* empty fn */
    }
  ...
  function updateMapVisuals(ui) {
    switch (settings.color.mode) {
      ...
      case "r":
        setAlpha.call(
          that,
          colorMapL2,
          toFixedNumeric(((ui.val("r") || 0) / 255) * 100, 4)
        );
        break;
      case "g":
        setAlpha.call(
          that,
          colorMapL2,
          toFixedNumeric(((ui.val("g") || 0) / 255) * 100, 4)
        );
        break;
      case "b":
        setAlpha.call(
          that,
          colorMapL2,
          toFixedNumeric(((ui.val("b") || 0) / 255) * 100)
        );
        break;
    }
    const a = ui.val("a");
    setAlpha.call(
      that,
      colorMapL3,
      toFixedNumeric(((255 - (a || 0)) * 100) / 255, 4)
    );
  }
  /**
   * @param {external:jQuery} ui
  ...
  function updateBarVisuals(ui) {
    switch (settings.color.mode) {
      ...
      case "s": {
        ...
        const saturatedColor = new Color({
          h: (hva && hva.h) || 0,
          s: 100,
        ...
        setAlpha.call(
          that,
          colorBarL5,
          toFixedNumeric(((255 - ((hva && hva.a) || 0)) * 100) / 255, 4)
        );
        break;
      }
      case "v": {
        ...
        setAlpha.call(
          that,
          colorBarL5,
          toFixedNumeric(((255 - ((hsa && hsa.a) || 0)) * 100) / 255, 4)
        );
        break;
      }
      case "r":
      ...
      case "b": {
        ...
        setAlpha.call(
          ...
          colorBarL2,
          hValue > vValue
            ? toFixedNumeric(((hValue - vValue) / (255 - vValue)) * 100, 4)
            : 0
        );
        setAlpha.call(
          that,
          colorBarL3,
          vValue > hValue
            ? toFixedNumeric(((vValue - hValue) / (255 - hValue)) * 100, 4)
            : 0
        );
        setAlpha.call(
          that,
          colorBarL4,
          toFixedNumeric((middle / 255) * 100, 4)
        );
        setAlpha.call(
          that,
          colorBarL5,
          toFixedNumeric(((255 - ((rgba && rgba.a) || 0)) * 100) / 255, 4)
        );
        break;
      }
      case "a": {
  ...
  function setImg(img, src) {
    if (
      isLessThanIE7 &&
      (src.includes("AlphaBar.png") ||
        src.includes("Bars.png") ||
        src.includes("Maps.png"))
    ) {
  ...
  function cancelClicked() {
    ...
    typeof cancelCallback === "function" &&
      cancelCallback.call(that, color.active, cancelButton);
  }
  /**
   *
   * @returns {void}
  ...
  function okClicked() {
    ...
    settings.window.expandable && hide.call(that);
    typeof commitCallback === "function" &&
      commitCallback.call(that, color.active, okButton);
  }
  /**
   *
   * @returns {void}
  ...
  function currentColorChanged(ui) {
    ...
    setAlpha.call(
      that,
      currentPreview,
      toFixedNumeric(((ui.val("a") || 0) * 100) / 255, 4)
    );
  }
  /**
   * @param {external:jQuery} ui
  ...
  function expandableColorChanged(ui) {
    ...
    setAlpha.call(
      that,
      iconAlpha,
      toFixedNumeric(((255 - ((va && va.a) || 0)) * 100) / 255, 4)
    );
    if (settings.window.bindToInput && settings.window.updateInputColor) {
      settings.window.input.style.backgroundColor =
        (hex && "#" + hex) || "transparent";
      settings.window.input.style.color =
        isNullish(va) || va.v > 75 ? "#000000" : "#ffffff";
    }
  }
  /**
   * @param {Event} e
  ...
  function show() {
    ...
    function attachIFrame() {
      ...
      pELem.style.width = parseFloat(
        getComputedStyle(table, null).width.replace("px", "")
      );
      pELem.style.height = parseFloat(
        getComputedStyle(container, null).height.replace("px", "")
      );
      pELem.style.opacity = 0;
      pELem.style.position = "absolute";
      pELem.style.left = getComputedStyle(container, null).getPropertyValue(
        "left"
      );
      pELem.style.top = getComputedStyle(container, null).getPropertyValue(
        "top"
      );
    }
    if (settings.window.expandable) {
  ...
  function initialize() {
    ...
    const controlHtml = `<table class="jPicker" id="jPicker-table" cellpadding="0" cellspacing="0">
      <tbody>
        ...
        <tr>
          <td rowspan="9"><h2 class="Title">${
            win.title || "config.jpicker_title"
          }</h2><div class="Map" id="Map"><span class="Map1" id="MMap1">&nbsp;</span><span class="Map2" id="MMap2">&nbsp;</span><span class="Map3" id="MMap3">&nbsp;</span><img src="${
      images.clientPath + images.colorMap.arrow.file
    }" class="Arrow"/></div></td>
          <td rowspan="9"><div class="Bar" id="Bar"><span class="Map1" id="Map1">&nbsp;</span><span class="Map2" id="Map2">&nbsp;</span><span class="Map3" id="Map3">&nbsp;</span><span class="Map4" id="Map4">&nbsp;</span><span class="Map5" id="Map5">&nbsp;</span><span class="Map6" id="Map6">&nbsp;</span><img src="${
            images.clientPath + images.colorBar.arrow.file
          }" class="Arrow"/></div></td>
          <td colspan="2" class="Preview" id="Preview">${i18next.t(
            "config.jpicker_new_color"
          )}<div id="preview-div"><span class="Active" id="Active" title="${i18next.t(
      "config.jpicker_tooltip_colors_new_color"
    )}">&nbsp;</span><span class="Current" id="Current" title="${i18next.t(
      "config.jpicker_tooltip_colors_current_color"
    )}">&nbsp;</span></div>${i18next.t("config.jpicker_current_color")}</td>
          <td rowspan="9" class="Button" id="Button"><input type="button" class="Ok" id="Ok" value="${i18next.t(
            "common.ok"
          )}" title="${i18next.t(
      "config.jpicker_tooltip_buttons_ok"
    )}"/><input type="button" class="Cancel" id="Cancel" value="${i18next.t(
      "common.cancel"
    )}" title="${i18next.t(
      "config.jpicker_tooltip_buttons_cancel"
    )}"/><hr/><div class="Grid" id="Grid"></div></td>
        </tr>
        <tr class="Hue">
          <td class="Radio"><label title="${i18next.t(
            "config.jpicker_tooltip_hue_radio"
          )}"><input type="radio" value="h"${
      settings.color.mode === "h" ? ' checked="checked"' : ""
    }/>H:</label></td>
          <td class="Text"><input type="text" maxlength="3" value="${
            all ? all.h : ""
          }" title="${i18next.t(
      "config.jpicker_tooltip_hue_textbox"
    )}"/>&nbsp;&deg;</td>
        </tr>
        <tr class="Saturation">
          <td class="Radio"><label title="${i18next.t(
            "config.jpicker_tooltip_saturation_radio"
      ...
      "config.jpicker_tooltip_saturation_textbox"
    )}"/>&nbsp;%</td>
        </tr>
        <tr class="Value">
          <td class="Radio"><label title="${i18next.t(
            "config.jpicker_tooltip_value_radio"
      ...
      "config.jpicker_tooltip_value_textbox"
    )}"/>&nbsp;%<br/><br/></td>
        </tr>
        <tr class="Red">
          <td class="Radio"><label title="${i18next.t(
            "config.jpicker_tooltip_red_radio"
          )}"><input type="radio" value="r"${
      settings.color.mode === "r" ? ' checked="checked"' : ""
    }/>R:</label></td>
          <td class="Text"><input type="text" maxlength="3" value="${
            all ? all.r : ""
          }" title="${i18next.t("config.jpicker_tooltip_red_textbox")}"/></td>
        </tr>
        <tr class="Green">
          <td class="Radio"><label title="${i18next.t(
            "config.jpicker_tooltip_green_radio"
          )}"><input type="radio" value="g"${
      settings.color.mode === "g" ? ' checked="checked"' : ""
    }/>G:</label></td>
          <td class="Text"><input type="text" maxlength="3" value="${
            all ? all.g : ""
          }" title="${i18next.t("config.jpicker_tooltip_green_textbox")}"/></td>
        </tr>
        <tr class="Blue">
          <td class="Radio"><label title="${i18next.t(
            "config.jpicker_tooltip_blue_radio"
          )}"><input type="radio" value="b"${
      settings.color.mode === "b" ? ' checked="checked"' : ""
    }/>B:</label></td>
          <td class="Text"><input type="text" maxlength="3" value="${
            all ? all.b : ""
          }" title="${i18next.t("config.jpicker_tooltip_blue_textbox")}"/></td>
        </tr>
        <tr class="Alpha">
          <td class="Radio">${
            win.alphaSupport
              ? `<label title="${i18next.t(
                  "config.jpicker_tooltip_alpha_radio"
          ...
          <td class="Text">${
            win.alphaSupport
              ? `<input type="text" maxlength="${
                  ...
                  all
                    ? toFixedNumeric((all.a * 100) / 255, win.alphaPrecision)
                    : ""
                }" title="${i18next.t(
                  "config.jpicker_tooltip_alpha_textbox"
                )}"/>&nbsp;%`
              : "&nbsp;"
          }</td>
        </tr>
        <tr class="Hex">
          <td colspan="2" class="Text"><label title="${i18next.t(
            "config.jpicker_tooltip_hex_textbox"
          )}">#:<input type="text" maxlength="6" class="Hex" value="${
      all ? all.hex : ""
    }"/></label>${
      win.alphaSupport
        ? `<input type="text" maxlength="2" class="AHex" value="${
            all ? all.ahex.substring(6) : ""
          }" title="${i18next.t("config.jpicker_tooltip_hex_alpha")}"/></td>`
        : "&nbsp;"
    }
        </tr>
      </tbody></table>`;
    if (win.expandable) {
      ...
      if (!that.querySelectorAll("div.jPicker.Container").length) {
        ...
        that
          .querySelector("div.jPicker.Container:last")
    ...
    setAlpha.call(
      that,
      currentPreview,
      toFixedNumeric((color.current.val("a") * 100) / 255, 4)
    );
    ...
    setTimeout(function () {
      ...
      setImg.call(
        that,
        preview.querySelector("#preview-div"),
    ...
    if (color.quickList && color.quickList.length > 0) {
      ...
      for (let i = 0; i < color.quickList.length; i++) {
        ...
        html +=
          ...
          (quickHex
            ? ""
            : "background-image:url(" + images.clientPath + "NoColor.png)") +
          (win.alphaSupport && alpha && alpha < 255
            ? ";opacity:" +
              toFixedNumeric(alpha / 255, 4) +
              ";filter:Alpha(opacity=" +
              toFixedNumeric(alpha / 2.55, 4) +
              ")"
            : "") +
          '">&nbsp;</span>';
      }
    ...
    if (win.expandable) {
      ...
      setAlpha.call(
        that,
        iconAlpha,
        toFixedNumeric(((255 - (all ? all.a : 0)) * 100) / 255, 4)
      );
      ...
      if (win.bindToInput && win.updateInputColor) {
        ...
        win.input.style.color =
          isNullish(all) || all.v > 75 ? "#000000" : "#ffffff";
      }
  ...
  const color = {
    active:
      (typeof settings.color.active).toString().toLowerCase() === "string"
        ...
        : new Color({
            ahex:
              !settings.window.alphaSupport && settings.color.active.val("ahex")
