...
import { getParents } from "./jgraduate/Util.js";
...
export const init = (canvas) => {
  ...
  svgCanvas.cloneSelectedElement = cloneSelectedElement;
  ...
  svgCanvas.moveToTop1StepSelectedElement = moveToTop1StepSelectedElem; // Repositions the selected element to the bottom in the DOM to appear on top
  svgCanvas.moveToBottom1StepSelectedElement = moveToBottom1StepSelectedElem; // Repositions the selected element to the top in the DOM to appear under other elements
 ...
 * Repositions the selected element to the bottom in the DOM to appear on top of
 * other elements.
 * @function module:selected-elem.SvgCanvas#moveToTop1StepSelectedElem
 * @fires module:selected-elem.SvgCanvas#event:changed
 * @returns {void}
 */
const moveToTop1StepSelectedElem = () => {
  const [selected] = svgCanvas.getSelectedElements();
  if (selected) {
    const t = selected;
    const oldParent = t.parentNode;
    ...
    if(oldNextSibling){
      t.parentNode.insertBefore(oldNextSibling,t)
    }
    // If the element actually moved position, add the command and fire the changed
    // event handler.
    if (oldNextSibling !== t.nextSibling) {
      svgCanvas.addCommandToHistory(
          new MoveElementCommand(t, oldNextSibling, oldParent, "top1step")
      );
      svgCanvas.call("changed", [t]);
    }
  }
 ...
 * Repositions the selected element to the bottom in the DOM to appear on top of
 * other elements.
 * @function module:selected-elem.SvgCanvas#moveToBottom1StepSelectedElem
 * @fires module:selected-elem.SvgCanvas#event:changed
 * @returns {void}
 */
const moveToBottom1StepSelectedElem = () => {
  const [selected] = svgCanvas.getSelectedElements();
  if (selected) {
    const t = selected;
    const oldParent = t.parentNode;
    ...
    let oldPrevSibling = t.previousSibling;
    if (oldPrevSibling.tagName === "title") {
      oldPrevSibling = oldPrevSibling.nextSibling;
    }
    // This can probably be removed, as the defs should not ever apppear
    // inside a layer group
    if (oldPrevSibling.tagName === "defs") {
      oldPrevSibling = oldPrevSibling.nextSibling;
    }

    t.parentNode.insertBefore(t, oldPrevSibling)
    // If the element actually moved position, add the command and fire the changed
    // event handler.
    if (oldNextSibling !== t.nextSibling) {
      svgCanvas.addCommandToHistory(
          new MoveElementCommand(t, oldNextSibling, oldParent, "bottom1step")
      );
      svgCanvas.call("changed", [t]);
    }
  }
 ...
 * @returns {void}
 */
const cloneSelectedElement = (x, y) => {
  const selectedElement = svgCanvas.getSelectedElements()[0];
  const currentGroup = svgCanvas.getCurrentGroup();
  let i;
  let elem;
  ...
  const drawing = svgCanvas.getDrawing();
  // clone each element and replace it within copiedElements
  elem = drawing.copyElem(selectedElement);
  (currentGroup || drawing.getCurrentLayer()).append(elem);
 ...
 * @returns {void}
 */
const deleteSelectedElements = (func) => {
  const selectedElements = svgCanvas.getSelectedElements();
  ...
  selectedElements.forEach((selected) => {
    if (selected) {
      ...
      batchCmd.addSubCommand(
          new RemoveElementCommand(elem, nextSibling, parent,"Delete " + selected.tagName,func)
      );
    }
  });
