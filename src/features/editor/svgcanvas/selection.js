...
import { getClosest } from './jgraduate/Util.js'
...
const clearSelectionMethod = (noCall) => {
  ...
  selectedElements.forEach((elem) => {
    ...
    elem.removeAttribute('editor_selected')
...
const getMouseTargetMethod = (evt) => {
  ...
  while (!mouseTarget?.parentNode?.isSameNode(svgCanvas.getCurrentGroup() || currentLayer)) {
    if (mouseTarget) {
      mouseTarget = mouseTarget.parentNode
    }
  }
...
const recalculateAllSelectedDimensions = () => {
  const text = svgCanvas.getCurrentResizeMode() === 'position' ? 'position' : 'size'
