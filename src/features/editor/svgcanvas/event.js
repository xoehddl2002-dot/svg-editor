...
import { findPos } from "./jgraduate/Util.js";
...
const mouseMoveEvent = (evt) => {
  ...
  switch (svgCanvas.getCurrentMode()) {
    case "select": {
      ...
      if (selected) {
        if (selectedElements.length == 1) {
          const isMove = selected.getAttribute("editor_move");
          const isSelected = selected.getAttribute("editor_selected");
          const targetId = evt.target.getAttribute("id")
          const isModifyBtn=targetId!="selectorGrip_modify_txt"&&targetId!="selectorGrip_delete_txt"
          ...
          if (
            ...
            svgCanvas.setCurrentResizeMode("position");
    ...
    case "zoom": {
      ...
      if (parent.tagName == "g" && evtTarget.tagName == "image") {
        //svgCanvas.pushGroupProperties(mouseTarget);
        //svgCanvas.pushGroupProperties(evtTarget.parent);
      }
      /*
      realX *= zoom;
      realY *= zoom;
  ...
  switch (svgCanvas.getCurrentMode()) {
    ...
    case "select":
      if (selectedElements[0]) {
        ...
        if (svgCanvas.getCurrentResizeMode() != "none") {
        ...
        */
        ...
        elem.setAttribute("editor_selected", "true");
        svgCanvas.getMobileBtnFunc()(false)
...
*/
  ...
  const parentGroupId = evtTarget.getAttribute("image_id") || evtTarget.getAttribute("shapes_id")
  if (parentGroupId) {
    const parentGroup = document.querySelector("g[image_id='" + parentGroupId + "'],g[shapes_id='" + parentGroupId + "']")
    if (parentGroup) {
      if (parentGroup.hasAttribute("image_id")) {
        const imageId = parentGroup.querySelector("[image_id='" + parentGroupId + "']")
        if (imageId.parentNode.hasAttribute("clip-path")) {
          mouseTarget = parentGroup.querySelector("[clip-path]");
...
const mouseDownEvent = (evt) => {
  ...
  switch (svgCanvas.getCurrentMode()) {
      ...
      if (mouseTarget !== svgRoot) {
        ...
        if (!rightClick) {
          ...
          if (parent.tagName == "g" && evtTarget.tagName == "image") {
            ...
            //svgCanvas.pushGroupProperties(evtTarget.parent);
            if (!svgCanvas.getCurrentGroup()) {
              mouseTarget = evtTarget;
              draw.setContext(parent);
              // No need to do the call here as it will be done on addToSelection
              if (!evt.shiftKey) {
                svgCanvas.clearSelection(true);
              }
              ...
              svgCanvas.setJustSelected(mouseTarget);
              svgCanvas.pathActions.clear();
              svgCanvas.selectorManager.requestSelector(mouseTarget).resize();
              //
            }
          }*/
        }
    ...
    case "resize": {
      ...
      svgCanvas.setInitBbox(getBBox($id("selectedBox")));
      const bb = {};
