...
import { mergeDeep } from "./jgraduate/Util.js";
...
// if it's a regular group, we have special processing to flatten transforms
    ...
    if (
      ...
      if (selected.tagName == "g") {
        if (selected.hasAttribute("image_id")) {
          const count = selected.querySelectorAll("image,defs > *").length;
          if (count > 0) {
            children = selected.querySelectorAll("image,defs > *");
          }
        }else if (selected.hasAttribute("add_svg")) {
          const count = selected.querySelectorAll("use[id]").length;
          if (count > 0) {
            children = selected.querySelectorAll("use[id]");
          }
        } else {
          return
          /*
            const count = selected.querySelectorAll("line,circle,ellipse,foreignObject,rect,image,text,polygon,polyline,path").length;
            if (count > 0) {
              children = selected.querySelectorAll("line,circle,ellipse,foreignObject,rect,image,text,polygon,polyline,path");
            }
            */
        }
      ...
      while (c--) {
        ...
        tx = 0;
        ty = 0;
        if (child.nodeType === 1 && child.transform) {
      ...
      if (tx !== 0 || ty !== 0) {
        ...
        if (selected.tagName == "g") {
          if (selected.hasAttribute("image_id")) {
            ...
            const count = selected.querySelectorAll("line,circle,ellipse,use,mask,foreignObject,rect,image,text,polygon,polyline,path").length;
            if (count > 0) {
              children = selected.querySelectorAll("line,circle,ellipse,use,mask,foreignObject,rect,image,text,polygon,polyline,path");
            }
