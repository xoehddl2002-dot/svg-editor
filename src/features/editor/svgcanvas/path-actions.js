...
import { transformPoint, snapToAngle, rectsIntersect, transformListToTransform } from './math.js'
import { assignAttributes, getElement, getRotationAngle, snapToGrid, getBBox } from './utilities.js'
    ...
    mouseDown(evt, mouseTarget, startX, startY) {
      ...
      if (svgCanvas.getCurrentMode() === 'path') {
        ...
        if (!drawnPath) {
          ...
          /* drawnPath = */ svgCanvas.setDrawnPath(
          ...
          if (clickOnPoint) {
            ...
            // const element = newpath; // Other event serviceHandlers define own `element`, so this was probably not meant to interact with them or one which shares state (as there were none); I therefore adding a missing `var` to avoid a global
            ...
            if (
              !(
                svgCanvas.getContainer() !== svgCanvas.getMouseTarget(evt) &&
                svgCanvas.getContainer().contains(svgCanvas.getMouseTarget(evt))
              )
            ) {
      ...
      if (!path.dragging) {
        ...
        if (!rubberBox) {
          rubberBox = svgCanvas.setRubberBox(svgCanvas.selectorManager.getRubberBandBox())
        }
        const zoom = svgCanvas.getZoom()
        assignAttributes(
          rubberBox,
        ...
        if (newPoint) {
          ...
          if (stretchy) {
            ...
            if (prev.pathSegType === 6) {
              ...
              svgCanvas.replacePathSeg(6, 1, [mouseX, mouseY, prevX * zoom, prevY * zoom, mouseX, mouseY], stretchy)
            } else if (firstCtrl) {
